    ...
    Dulta.renderMe('video-self-view', function() {
      ...
      Dulta.joinRoom({ name: 'call-code-'+callCode }, function (room) {
        ...
        room.on('call:accepted', function(call) {
          ...
          call.participants.forEach(function(index, participant) {
            if (participant.isMe) {
              ...
              Dulta.whiteboards.create('video-self-view', participant);
              return;
            }
            ...
            Dulta.whiteboards.create(videoDivId, participant);

            // First call to Dulta.colorManager.getColor will assign a new color to this participant
            $('#' + videoDivId).css('border-color', Dulta.colorManager.getColor(participant.id));
        ...
        room.on('participants:leave', function(data) {
          ...
          Dulta.whiteboards.remove(participant_id);
        });
...
function initTextChat(room) {
  ...
  room.on('text:received', function(msg) {
    ...
    if (msg.sender == Dulta.profiles.getAt(0)._id) {
      ...
      container.append($('<div class="row text-right chat-message"><span class="label label-info top-right-block" style="background-color: ' +
        Dulta.colorManager.getColor(msg.sender) +';">' + msg.content.text + '</span></div>'));
    }
  ...
  $('#chat-message-input').keypress(function() {
    if (Date.now()-lastTypingEventInterval > lastTypingEventFired) {
      ...
      var message = {
        to: room.profileIds,
        from: Dulta.profiles.getAt(0)._id,
        content: 'is-typing-event'
      }
      Dulta.sendCustomMessage(message);
    }
  });

  // Listen for is-typing-event
  Dulta.on('custom:received', function(msg) {
    if (msg.sender != Dulta.profiles.getAt(0)._id && msg.content == 'is-typing-event') {
      ...
      $('#chat-message-is-typing > span').css('background-color', Dulta.colorManager.getColor(msg.sender));
  ...
  Dulta.on('auth:multiconnecton', function(msg) {
...
$(document).ready(function() {
  ...
  // Check Dulta browser capabilities
  Dulta.getCapabilities(function(error, capabilities) {
    // Hide progressbar
